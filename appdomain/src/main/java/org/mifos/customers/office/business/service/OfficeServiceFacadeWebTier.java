...
public class OfficeServiceFacadeWebTier implements OfficeServiceFacade {
    ...
    @Override
    public OfficeFormDto retrieveOfficeFormInformationForfa(Short officeLevelId) {
        ...
        try {
            ...
            List<OfficeDetailsDto> officeLevels = new OfficePersistence().getActiveLevelsForfa();
            for (OfficeDetailsDto officeDetailsDto : officeLevels) {
                String levelName = ApplicationContextProvider.getBean(MessageLookup.class).lookup(officeDetailsDto.getLevelNameKey());
                officeDetailsDto.setLevelName(levelName);	
            }
