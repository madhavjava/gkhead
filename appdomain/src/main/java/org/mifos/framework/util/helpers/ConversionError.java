...
public enum ConversionError {
    ...
    EXCEEDING_NUMBER_OF_DIGITS_AFTER_DECIMAL_SEPARATOR_FOR_MONEY_FA {
            @Override
            public String toLocalizedMessage(MifosCurrency currency) {
                String errorText = super.toLocalizedMessage(currency);
                return errorText.replaceFirst("%s", AccountingRules.getMultpleDigitsAfterDecimal(currency).toString());
            }},
