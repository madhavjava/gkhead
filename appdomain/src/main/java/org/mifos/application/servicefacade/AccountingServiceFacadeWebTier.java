...
public class AccountingServiceFacadeWebTier implements AccountingServiceFacade {
	...
	@Override
	public List<GLCodeDto> accountHeadGK(String glCode) {
		List<GLCodeDto> accountingDtos = null;
		accountingDtos = accountingDao.findAccountHeadGlCodesGK(glCode);
		return accountingDtos;
	}
	@Override
	public List<GLCodeDto> accountHeadAll(String glCode) {
		List<GLCodeDto> accountingDtos = null;
		accountingDtos = accountingDao.findAccountHeadGlCodesAll(glCode);
		return accountingDtos;
	}
	@Override
	...
	@Override
	public List<GLCodeDto> insuranceRelatedAccounts() {
		List<GLCodeDto> accountingDtos = null;
		accountingDtos = accountingDao.findinsuranceRelatedAccounts();
		return accountingDtos;
	}
	...
	@Override
	public List<ViewTransactionsDto> getAccountingTransactionsByAccountHead(Date toTrxnDate,Date fromTrxnDate,
			int startRecord, int numberOfRecords,String glcode) {
		List<ViewTransactionsDto> accountingTransactions = null;
		accountingTransactions = accountingDao.findAccountingTransactionsByAccountHead(
				toTrxnDate,fromTrxnDate, startRecord, numberOfRecords,glcode);
		return accountingTransactions;
	}
	...
	GlMasterBO getGlMasterBO(MisProcessingTransactionsDto dto, Short createdBy) {
		...
		int traxnfinid=dto.getFinActionId();
		if(traxnfinid==7){
			glDetailBOList.add(new GlDetailBO(dto.getGlCredit(), dto.getAmount(),
					"debit", "", null, "", "",dto.getTransactionNotes()));
		}else{
		glDetailBOList.add(new GlDetailBO(dto.getGlCredit(), dto.getAmount(),
	...
	@Override
	public List<GLCodeDto> findTotalGlAccountsAssestsAndLibialities() {
		List<GLCodeDto> accountingDtos = null;
		accountingDtos = accountingDao.findGlCodesAssetsAndLibialities();
		return accountingDtos;
	}
