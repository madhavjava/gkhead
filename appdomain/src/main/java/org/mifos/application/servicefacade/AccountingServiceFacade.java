...
public interface AccountingServiceFacade {
	...
	List<ViewTransactionsDto> getAccountingTransactionsByAccountHead(
			Date toTrxnDate, Date fromTrxnDate, int startRecord,
			int numberOfRecords, String glcode);

	List<GLCodeDto> insuranceRelatedAccounts();

	List<GLCodeDto> accountHeadAll(String glCode);

	List<GLCodeDto> findTotalGlAccountsAssestsAndLibialities();
	List<GLCodeDto> accountHeadGK(String glCode) ;
}
